...
function checkResults(e){
    ...
    switch(type){
        ...
        case "state":
        case "add1":
                    if (value.length < minLength || value.length > maxLength || !value.trim().match(regexGeneralText)) {
                        document.getElementById(em).style.display = "block";
                        this.style.border = "2px solid red";
                        if (type === "state") isStateValid = false;
                        if (type === "add1") isStreetAddressValid = false;
                    } else {
                        document.getElementById(em).style.display = "none";
                        this.style.border = "";
                        if (type === "state") isStateValid = true;
                        if (type === "add1") isStreetAddressValid = true;
                    }
                    break;
        case("zipcode"):
